...
RSpec.describe ExchangeRateFetcher do
  ...
  describe "#call" do
    context "when rate is cached" do
      before do
        create(:exchange_rate, base_currency: "USD", quote_currency: "BRL", rate: 52_000, fetched_on: date)
      end
    ...
    context "when API fails", vcr: { cassette_name: "exchange_rate/not_found" } do
      ...
      it "returns nearest cached rate as fallback" do
        create(:exchange_rate, base_currency: "USD", quote_currency: "BRL", rate: 51_000, fetched_on: 1.day.ago)

        result = described_class.new(date: date).call
        expect(result).to eq(51_000)
      end
    end
  end
end
